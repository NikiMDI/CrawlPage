...
Чтобы узнать `Content-Type`, crawler должен начать HTTP-запрос и получить заголовки. Поэтому фактических диагностических HTTP-запросов может быть больше, чем `Pages checked`. `Pages checked` — это счётчик результатов, принятых в бюджет страниц, а не счётчик всех сетевых операций. Тело успешного non-HTML-ответа не считывается и HTML-парсеру не передаётся. PDF по-прежнему отсекается до запроса.

Scheduler единолично управляет ограниченным LIFO-стеком, глубинами, уже запущенными URL и результатами. `--max-queue` ограничивает число URL, ожидающих worker; `--concurrency` — число активных заданий. Канал `jobs` небуферизован, поэтому в scheduler не больше `max-queue` URL ожидают и не больше `concurrency` выполняются; их сумма задаёт верхнюю границу текущей работы. При переполнении новая ссылка не кладётся в `frontier`; если тот же URL встретится позже при свободном месте, он может быть поставлен в очередь. При `--concurrency 1` сохраняется строгий последовательный DFS, а при большем значении используется DFS-приоритет с параллельной обработкой нескольких ветвей.

В сводке печатаются `Maximum queue`, `Peak queue`, `Queue limit reached` и `Skipped by queue`. Последний счётчик показывает число уникальных URL, которые остались отброшенными: URL, успешно поставленный в очередь при повторном обнаружении, из этого счётчика удаляется.
