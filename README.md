...
Пятый этап выполняет ограниченный конкурентный обход графа, классифицирует конечный результат проверки каждого внутреннего URL и вручную сохраняет redirect-цепочки:
...
Потокобезопасный fetch-кэш использует отдельную singleflight-запись на один HTTP URL. Если direct-ссылка и redirect-цепочка одновременно приходят к одному target, реальный запрос выполняется один раз, а остальные workers получают тот же результат. Подробности находятся в [документации этапа 5](docs/crawler/stage-05.md).
...
## Структура проекта после этапа 5
