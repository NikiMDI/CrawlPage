...
Общий fetch-кэш гарантирует, что один нормализованный внутренний URL запрашивается не более одного раза, даже если он встречается и как обычная ссылка, и внутри redirect-цепочки. Каждый новый фактически запрошенный внутренний URL входит в `--max-pages`; внешний target лимит не расходует. PDF не запрашивается: внутренняя `<a href="*.pdf">` записывается в `Skipped`, redirect на PDF останавливается до запроса цели, а PDF нельзя передать как стартовый URL.
